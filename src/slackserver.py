...
def now():
    ...
    return timestamp - (6*3600)# +(24*3600)

jsonp_plugin = JSONPlugin()

def render_posts(comment,url,data):
    logger.debug(url)
    r = requests.get(url)
    
    if r.status_code == 200:
        events = json.loads(r.content)
        if len(events.get("hits").get("hits")) ==0:
            comment = "no events, search for events: https://test.gbme.nl/sxsw"
        logger.debug(events)
        responsedata = {
            "text": "@"+data.get("user_name")+" "+comment,
            "attachments": [
                ]

            }
        for event in events.get("hits").get("hits"):
            event_source = event.get("_source")
            responsedata["attachments"].append({
...
def slack_command():
    try:
        ...
        utime = now()
        date = datetime.fromtimestamp(utime)
        logger.debug(date)

        logger.debug(utime)
        if data.get("command") == "/sxsw" and words[0] == "now":  
            url = "http://127.0.0.1:9200/sxswevents/event/_search?q=attendees:" + data.get("user_name","") + " AND utime_start:<"+str(utime*1000)+" AND utime_start:>"+str((utime-7200)*1000)+"&sort=utime_start:asc&size=10"
            render_posts("Your current event(s):",url,data)
        if data.get("command") == "/sxsw" and words[0] == "next":  
            url = "http://127.0.0.1:9200/sxswevents/event/_search?q=attendees:" + data.get("user_name","") + " AND utime_start:>"+str(utime*1000)+" AND utime_start:<"+str((utime+3600)*1000)+"&sort=utime_start:asc&size=10"
            logger.debug(url)
            render_posts("Your next event(s):",url,data)
